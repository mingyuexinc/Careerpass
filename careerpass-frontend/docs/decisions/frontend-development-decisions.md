...
前端 MVP 先以 Mock 驱动完成页面和交互；从 S-06 开始，已锁定 Integration Contract 的页面按切片接入真实后端并进行联调，未接入页面继续保持 Mock 边界。
...
求职目标配置页可以在用户登录后进入并保存目标，不等待简历解析或岗位 JD 完成；简历和画像条件在 S-07 启动时校验，岗位条件由 S-08 在匹配前校验。

S-07 任务页采用左侧“目标配置与启动”、右侧“当前目标只读查看”的双栏结构；页面状态矩阵和编码前检查清单见 [`../development/slice-07-agent-start.md`](../development/slice-07-agent-start.md)。右侧复用 S-06 当前目标查询，不创建第二套目标业务能力。
...
| 真实后端接口联调 | 按具体 Integration Contract 接入真实后端并执行前后端联调；S-08 使用既有启动接口和 `GET /api/v1/applications/current`，S-09 使用 HR 岗位/投递查询和状态更新接口，未接入的页面继续使用 Mock 数据访问层 |
| 真实文件存储和简历解析 | 只模拟上传和解析状态 |
| 真实 Agent 调度、匹配和投递 | 通过固定数据和状态变化表达结果 |
| 实时消息推送 | 使用前端模拟回复和刷新 |
| 多轮投递 | 只展示单轮投递 |
| 多个求职目标和多份简历并行使用 | 只验证一个求职目标和当前简历 |
| 用户主动暂停或终止 Agent | 只由系统在 Offer 达标或本轮全部岗位筛选完成且 Application 数量为 0 后结束 |
...
| 决策 | 内容 |
| --- | --- |
| Match 可见范围 | Match 独立持久化，但前端不查询未投递 Match |
| 进度页数据 | 只消费当前 Candidate 的 Application |
| 推荐信息 | Application 展示 `matchScore` 和 `recommendationReason` |
| 启动交互 | 不新增匹配命令，不展示匹配中间过程；启动成功后进入进度页查看结果 |
| 零投递 | 至少一个岗位已进入算法且 Application 为 0 时展示“本轮暂未产生匹配结果”；无有效岗位时展示“岗位尚未准备完成” |
...
| 求职目标 | 未创建、已保存但待启动条件、可启动、运行中、已结束；运行中和已结束时目标只读 |
...
| 约束 | 前端表现 |
| --- | --- |
| 简历解析未成功 | Agent 启动按钮禁用；不阻止保存求职目标，页面不展示匹配资格详情 |
| 求职目标未创建 | 展示未满足条件，不能启动 Agent |
| 求职目标已保存 | Agent 未启动时可以更新同一个当前目标 |
| Agent 运行中 | 禁止替换当前简历；右侧当前求职目标展示为已锁定 |
| Agent 已结束 | 启动按钮保持禁用，历史记录继续展示 |
| Agent 运行中或已结束 | 求职目标不可修改 |
| Agent 已结束 | 启动按钮保持禁用，历史记录继续展示 |
| 投递轮次为 0 | 不展示虚构的投递记录，展示空状态 |
| HR 修改状态 | 只影响当前岗位下当前候选人的一条投递记录 |
| HR 投递信息 | 只展示岗位名称、公司名称、候选人姓名和当前投递进度；不展示联系方式、简历原文、匹配分数、推荐理由、沟通全文或其它候选人资料 |
| HR 投递查询范围 | 只读取当前 HR 所有未删除岗位下的当前首轮投递记录 |
| HR 状态选择 | 只提供当前状态之后的阶段和流程终止，不展示非法回退选项；Offer 和流程终止记录不可修改 |
| Offer 达标后 | Agent 结束且不可重新启动；其它未终态投递记录仍允许继续推进 |
...
| 岗位 | 至少 1 个已上传岗位 |
| 简历 | 当前任务绑定 1 份简历 |
| 求职目标 | 1 个当前目标，不绑定具体简历；任务页右侧只读展示当前目标 |
| 投递轮次 | 只验证首轮 |
| 投递记录 | 由开启投递轮次决定是否展示 |
| 会话 | 至少 1 个可沟通会话 |
| 消息 | 固定初始消息，可追加验证消息 |
...
- [ ] HR 投递进度页只展示四项已裁决的业务信息。
- [ ] Offer 达标后，其它未终态投递记录仍可以继续更新。
