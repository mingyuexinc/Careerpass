...
求职目标配置页可以在用户登录后进入并保存目标，不等待简历解析或岗位 JD 完成；简历、画像和岗位条件在启动 Agent 时统一校验。
...
| 真实后端接口联调 | S-06 起按具体 Integration Contract 接入真实后端并执行前后端联调；尚未接入的页面继续使用 Mock 数据访问层 |
...
| 求职目标 | 未创建、已保存但待启动条件、可启动、运行中、已结束 |
...
| 约束 | 前端表现 |
| --- | --- |
| 简历解析未成功 | Agent 启动按钮禁用；不阻止保存求职目标 |
