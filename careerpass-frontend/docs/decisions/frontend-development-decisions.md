...
> 状态：前端 MVP 开发目标草案
>
> 依据：`docs/product/frontend-demo-flow.md`

## 1. 文档目的

本文档定义 Careerpass 前端项目当前阶段的开发目标、交付边界和基本开发决策，为页面规格、Demo 数据、前端架构和开发规范提供统一依据。

本版本只针对前端项目和可重复演示的 MVP，不以解决后端系统设计问题或交付生产级全链路为目标。

## 2. 项目定位

Careerpass 前端的当前目标是：将已有 HTML 原型转化为结构清晰、可维护、可重复演示、便于后续接入真实数据的正式前端 MVP。
...
在不依赖当前后端文档和真实后端能力的前提下，完成一个能够稳定演示求职者侧和 HR 侧核心闭环的前端 MVP。
...
| 不包含内容 | 处理方式 |
| --- | --- |
| 真实账号体系和复杂注册 | 使用固定 Demo 账号，注册只保留扩展入口 |
| 真实后端接口联调 | 本期使用 Mock 数据访问层，保留后续替换边界 |
| 真实文件存储和简历解析 | 只模拟上传和解析状态 |
| 真实 Agent 调度、匹配和投递 | 通过固定数据和状态变化表达结果 |
| 实时消息推送 | 使用前端模拟回复和刷新 |
| 多轮投递 | 只展示单轮投递 |
| 多个求职目标和多份简历并行使用 | 只演示一个求职目标和当前简历 |
...
详细数据放在 `docs/development/demo-data.md`，本文件只记录范围决策。
...
后续演进不得直接扩大本期范围；新增能力应先更新本文件中的目标、范围和验收标准。
