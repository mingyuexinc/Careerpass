...
| 简历具备匹配资格 | `matchingReadiness = ready` | 继续判断岗位和目标；未满足时展示“业务字段缺失”并禁用启动按钮 |
| 存在可用结构化 JD | 至少一个岗位形成可供匹配的结构化快照 | 启动按钮可用；未满足时展示岗位准备未完成并禁用启动按钮 |
...
| --- | --- | --- | --- |
| `not_started` | 目标未创建或启动条件未全部满足 | 目标可以已保存；简历、画像或岗位条件可能仍未完成 | 启动按钮禁用 |
| `ready` | 目标已创建、简历解析成功、画像具备匹配资格且存在可用结构化 JD | 目标状态为可启动 | 启动按钮可用 |
| `running` | 用户点击启动 | S-07 将当前简历绑定到投递轮次并开启运行 | 展示运行中 |
