...
当前版本不要求 HR 上传的文件真实解析为结构化岗位；页面只需要展示上传成功和岗位摘要。

岗位上传数据在内部保留 `fileName`、`version` 和 `uploadedAt`，但正式页面不展示真实上传文件名。`jobs` 保存全部已上传岗位 JD，`currentJob` 指向最新上传记录。HR 岗位上传页使用列表卡片：左侧固定为“JD”，右侧第一行按“岗位名称 · 公司名称 · 工作地点 · 薪资”组合本地岗位数据，第二行展示“版本 {n} · 上传于 {时间}”。不得在其下方重复展示岗位摘要圆角卡片。重新上传或分批上传岗位 JD 时追加记录，最新记录成为当前岗位；本版本不提供编辑操作，但支持通过 `deleteJob(id)` 删除单条记录。

一次选择的多份岗位文件通过 `uploadJobs(files)` 在一次本地数据操作中追加；文件顺序决定版本递增顺序，最后一份文件成为 `currentJob`。
