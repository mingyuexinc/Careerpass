...
| 求职者上传 PDF 简历并展示解析和匹配资格状态 | 生产级文件中心、扫描件/OCR 和受保护 PDF 处理 |
...
| 3 | 求职者 | 等待简历解析完成 | 解析成功且业务字段完整时满足 Agent 启动前置条件；字段缺失时显示不可匹配 |
| 4 | 求职者 | 创建求职目标 | 目标配置完成 |
...
| 3 | 资料页 | 简历正在解析 | 刷新或等待状态变化 | 解析成功或失败 | 展示解析终态和匹配资格 |
| 4 | 资料页 | 用户已登录 | 上传其它求职资料 | 保存资料就绪状态 | 展示资料列表，不执行解析 |
| 5 | 任务页 | 简历解析成功 | 填写目标 Offer 数、岗位名称和过滤条件 | 保存求职目标 | 展示目标创建成功 |
| 6 | 任务页 | 简历解析成功、画像具备匹配资格、目标已创建 | 点击启动 Agent | Agent 开始执行匹配和投递 | 展示 Agent 运行中 |
...
| 简历解析成功且字段完整 | 展示可用简历，允许创建求职目标并满足匹配准入 |
| 简历解析成功但业务字段缺失 | 展示“业务字段缺失”，允许查看解析结果但 Agent 启动按钮禁用 |
...
| 规则 | 说明 |
| --- | --- |
| 简历与其它资料分开 | 简历需要解析，其它资料当前版本只展示上传就绪 |
| 每次上传一份资料 | 当前版本不验证批量上传 |
| 求职目标与简历 | 当前版本按一对一关系验证 |
...
| 任务页 | 点击启动 Agent | 简历解析成功、画像具备匹配资格且目标已创建 | Agent 进入运行中 |
...
- [ ] 简历解析未成功或画像不具备匹配资格时，Agent 启动按钮不可用。
- [ ] 求职者可以创建求职目标。
- [ ] 求职目标创建后，页面展示启动条件。
- [ ] 满足条件后可以启动 Agent。
- [ ] Agent 启动后展示运行中。
