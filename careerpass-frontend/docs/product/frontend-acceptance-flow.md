...
| 岗位未准备完成 | 没有成功解析的有效 JD 时不创建 Agent 运行，进度页展示“岗位尚未准备完成”；部分岗位失败时显示未参与数量 |
| Agent 已筛选但无投递 | 至少一个有效岗位进入算法且 Application 数量为 0 时，进度页展示“本轮暂未产生匹配结果”；未形成投递记录的匹配结果不展示 |
...
| 3 | 沟通页 | S-08 已创建 Application 并自动初始化 Conversation | 选择岗位和候选人 | 加载当前会话文本消息 | 展示聊天窗口 |
| 4 | 沟通页 | 当前会话可用 | 输入消息并发送 | S10-01 基于绑定 Resume-derived facts 回复；S10-02 资料请求按文件名语义返回一条 `content` 为空的 Agent 消息和一个附件 | S10-02 只展示仿微信接收文件效果的附件卡片，不展示“已为你找到”等额外提示语；失败保留输入并可重试 |
...
| S10-01 安全投影 | HR 只看到消息正文、发送主体、状态和时间，不展示 Resume 原文、fact_refs、Prompt、模型原始响应或内部定位 |
| S10-02 附件投影 | HR 只看到仿微信文件接收效果的附件卡片、文件名、格式、大小、创建/过期时间和下载状态；成功时不显示额外提示语；不展示 CandidateDocument ID、对象定位、正文或预览 |
| S10-02 下载 | 7 天有效期内可重复下载，不需要额外确认或重新登录；服务端仍校验当前 HR、Application 和 Conversation 归属 |
...
- [ ] 解析任务进入 `succeeded` 或 `failed` 终态且 Agent 尚未启动时，求职者可以逻辑删除当前简历；解析中、Agent 运行中或已结束时删除操作不可用并显示原因。
- [ ] `matching_ready` 和 `matching_not_ready` 都表示解析完成，不改变当前简历的删除资格；新上传的不同内容简历在 Agent 尚未启动时自动成为当前简历，解析未完成时 Agent 启动按钮保持禁用。
...
- [ ] 成功保存的附加资料在 Agent 未启动、运行中或已结束时均可逻辑删除；删除后从资料列表移除且不再用于新的资料检索，已发送附件在 7 天内继续可下载。
...
S-11 的删除操作不提供回收站或恢复入口；删除成功后页面移除对应资料并刷新列表或展示空状态。删除行为的权限、幂等和审计结果以 S-11 Integration Contract 为准。
...
- [x] JD 解析任务进入 `succeeded` 或 `failed` 终态且匹配尚未开始时，HR 可以逻辑删除岗位；解析中或匹配已开始时删除操作不可用并显示原因。
- [x] 岗位删除成功后从当前可用岗位列表移除；没有其他可用岗位时展示岗位空状态。
- [ ] HR 退出并重新登录后，岗位上传卡片仍展示已保存岗位的原始文件名。
- [ ] 存在已发起沟通时，HR 可以进入对应会话。
- [ ] HR 可以查看完整消息记录。
- [ ] HR 发送消息后展示发送中和 Agent 回复状态。
- [ ] 消息失败时可以重试。
- [x] S10-01 真实联调：HR 登录进入 `/hr/conversations`，空 Conversation 可见；经历存在性问题明确显示“没有相关经历”的 Agent 回答，事实问题显示正式 Agent 回答，超范围问题显示受控模板。
- [x] S10-01 发送中、失败重试、稳定 `client_message_id` 和安全消息投影已由前端测试及真实 API 联调覆盖。
- [x] S10-02 后端资料请求、单附件、重复下载、下载失败和过期状态已联调通过；成功提示语已移除并完成仿微信文件卡片整改回归。
- [ ] HR 可以按岗位查看投递记录。
