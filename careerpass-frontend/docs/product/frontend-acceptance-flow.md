...
stateDiagram-v2
    ...
    NotStarted --> Ready: 目标、简历、画像和岗位条件满足
...
| 6 | 任务页 | 目标已创建、简历解析成功、画像具备匹配资格且存在可用结构化 JD | 点击启动 Agent | S-07 绑定当前简历并开始执行匹配和投递 | 展示 Agent 运行中 |
...
| 简历解析成功且字段完整 | 展示可用简历；目标和可用结构化 JD 等启动条件满足后可启动 Agent |
| 简历解析成功但业务字段缺失 | 展示“业务字段缺失”，允许创建或更新目标但 Agent 启动按钮禁用 |
| 简历未上传或仍在解析 | 允许保存求职目标；Agent 启动按钮禁用 |
| 求职目标未创建 | 展示待完成条件，启动按钮不可用 |
| 求职目标已保存但岗位或简历条件未满足 | 展示已保存目标和未满足的启动条件，启动按钮禁用 |
...
| 任务页 | 点击启动 Agent | 目标已创建、简历解析成功、画像具备匹配资格且存在可用结构化 JD | S-07 绑定当前简历，Agent 进入运行中 |
...
- [ ] 简历解析未成功、画像不具备匹配资格或没有可用结构化 JD 时，Agent 启动按钮不可用。
- [ ] 求职者可以在简历解析完成前创建求职目标。
