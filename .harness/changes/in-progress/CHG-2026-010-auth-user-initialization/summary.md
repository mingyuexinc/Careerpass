...
- `careerpass-backend/app/api`：Bearer 身份解析与认证路由。
...
- Redis 认证限流已按 2026-07-31 的 MVP 范围裁决移除；认证模块不以 Redis 作为直接运行时依赖。
- `auth_sessions`、Refresh Token 及其迁移仍为 Deferred，不得成为 MVP 阻塞项。

## 当前状态

本模块已补建并通过阶段 1 需求分析与阶段 2 PostgreSQL 预验证；范围变化使历史阶段 3 及后续记录须按当前边界复核。尚未完成发布审批与实际上线，因此本变更包保持 `in-progress`。发布后方可迁移至 `released`。
