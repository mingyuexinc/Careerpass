...
**项目状态：** 已搭建求职者、HR 与求职 Agent 协作的核心业务链路，覆盖简历、岗位、匹配、投递、沟通和进度管理等主要模块。
...
- **对求职者：** 集中管理简历、求职资料和求职目标，统一查看岗位匹配、投递结果和招聘进度，减少重复整理和跟进成本。
- **对 HR：** 快速接收岗位 JD，查看授权范围内的候选人投递，在岗位上下文中与 Agent 沟通，并按合法招聘阶段推进投递记录。
- **对智能应用设计：** 将 Agent 放入真实业务流程中，通过结构化数据、前置条件、权限校验和状态机约束 Agent 行为，而不是只提供一个独立聊天窗口。
- **对工程实践：** 通过前后端分层、异步任务、幂等处理和 Integration Contract，将 AI 能力接入可维护、可审计的业务系统。
...
- [跨前后端业务基线](docs/business/business-baseline.md)
- [跨端 Integration 文档](docs/integration/README.md)
- [前端项目说明](careerpass-frontend/README.md)
- [后端项目说明](careerpass-backend/README.md)
