...
| 执行日期 | 执行人 | 环境 | 场景/步骤 | 预期 | 实际 | 结果 | 证据 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `<日期>` | `<角色>` | `<环境>` | `<内容>` | `<内容>` | `<内容>` | `通过/失败/阻塞` | `<测试、截图或关联 ID>` |
...
| 问题 ID | 复现步骤 | 预期 | 实际 | 分类 | 回退 Gate | 整改结果 | 回归结果 | 验收结果 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `<IS-问题-001>` | `<内容>` | `<内容>` | `<内容>` | `<分类>` | `<Gate>` | `<待整改/已完成>` | `<未验证/通过/失败>` | `<仅由开发者填写：通过/不通过/待验收>` |
