...
| `BF-FLOW-004` | `confirmed` | S-07 只有在当前求职目标已创建、当前简历解析成功、候选人画像具备岗位匹配资格且至少一个当前岗位 JD 已成功解析时才允许启动；岗位子集过滤和结果生成仍归属 S-08。 |
| `BF-FLOW-005` | `confirmed` | 岗位 JD 未完成解析时不得进入匹配；没有任何成功解析的有效岗位时，S-07 返回明确的 `409`，不创建 Agent 运行或 `no_match` 结果。 |
| `BF-FLOW-006` | `confirmed` | Offer 数量达到求职目标中的目标数量，或至少一个有效岗位进入算法且本轮全部有效岗位均已筛选、Application 数量为 0 时，当前 Agent 运行结束。 |
| `BF-FLOW-007` | `confirmed` | 只有至少一个成功解析的岗位真正进入本轮算法后，全部岗位均未形成投递记录时才产生 `no_match`；解析未完成或失败不得被误报为匹配无结果。 |
...
| `BF-FLOW-016` | `confirmed` | S-06 求职目标配置可以与 S-02/S-03 岗位准备和 S-04 简历解析并行；简历、画像和至少一个成功解析岗位条件由 S-07 启动前校验，具体可匹配岗位集合由 S-08 继续筛选。 |
...
| `BF-RULE-036` | `confirmed` | 当至少一个有效岗位进入算法、全部有效岗位均已筛选且本轮 Application 数量为 0 时，Agent 以 `no_match` 结束；没有有效岗位时阻断启动，不创建空运行。 |
...
| `BF-RULE-057` | `confirmed` | S10-02 只基于 CandidateDocument 文件名进行确定性语义匹配，使用标准化和受控关键词/别名，不读取文件内容、不进行 OCR/Embedding、不调用 LLM；“学籍证明”“学籍材料”“学籍验证报告”及带“材料”的同类表述归入同一受控资料意图；每次资料请求最多交付一个资料，主演示 Fixture 保证只有一个符合项。 |
...
| `BF-STATE-002` | `confirmed` | S-07 的“可启动”条件是求职目标已创建、当前简历解析成功、候选人画像具备岗位匹配资格且至少一个有效岗位 JD 已成功解析；具体岗位筛选仍由 S-08 执行。 |
| `BF-STATE-003` | `confirmed` | 用户启动后 Agent 进入“运行中”；运行中不能替换当前简历，也不能重复启动当前任务。 |
| `BF-STATE-004` | `confirmed` | Offer 数量达到目标，或至少一个有效岗位进入算法且本轮 Application 数量为 0 后，Agent 进入“已结束”；无投递的 `no_match` 运行在岗位状态改善后允许创建新的运行，保留旧运行记录。 |
...
| `BF-STATE-011` | `confirmed` | S-03 异步失败区分存储暂不可用、临时技术失败、可读内容非法和核心字段缺失：前两类有限次自动重试，耗尽后进入手动处理；非法内容和核心字段缺失直接进入手动处理且不形成可匹配快照。 |
