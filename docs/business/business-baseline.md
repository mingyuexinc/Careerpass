...
| `BF-FLOW-004` | `confirmed` | 求职 Agent 只有在当前求职目标已创建、当前简历解析成功、候选人画像具备岗位匹配资格且存在可用结构化岗位 JD 时才具备启动条件。 |
...
| `BF-FLOW-016` | `confirmed` | S-06 求职目标配置可以与 S-02/S-03 岗位准备和 S-04 简历解析并行；岗位 JD、简历和画像条件在 S-07 启动 Agent 时统一校验。 |
...
| `BF-RULE-015` | `confirmed` | 当前版本使用不依赖外部服务的简化岗位匹配；满足匹配资格的简历在当前构造岗位池中保证至少形成一条匹配结果，不实现匹配服务失败或空结果分支。 |
...
| `BF-STATE-002` | `confirmed` | “可启动”的业务条件是求职目标已创建、当前简历解析成功、候选人画像具备岗位匹配资格且存在可用结构化岗位 JD。 |
...
| `BF-SCOPE-013` | `confirmed` | 当前版本的岗位匹配使用本地简化算法，不依赖外部匹配服务；对具备匹配资格的简历保证产生至少一条匹配结果。匹配结果展示范围另行裁定。 |
...
| 事项 | 当前状态 | 影响 | 裁决位置 |
| --- | --- | --- | --- |
| 匹配结果展示范围 | `pending` | S-08 输出内容、求职者匹配结果页和投递记录选择 | S-08 Slice Design / Integration Contract |
