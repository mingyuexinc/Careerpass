...
| 后端状态 | `pending`；完成实现和后端验证后改为 `backend_ready` |
| 跨端交付状态 | `draft`；完成真实场景后改为 `integration_delivered` |
| 最小演示 | 提交受控本地 Markdown JD 路径，查询 `task_id`，读取成功 `fields` |
...
开发环境的受控 JD 根目录使用 `S03_JD_ROOT` 配置；未显式配置时，测试和内部验证默认使用 [`tests/fixtures/job_descriptions/`](../../../tests/fixtures/job_descriptions/)。该目录只存放脱敏构造 JD，不作为生产对象存储。
...
| AsyncTaskRun | 保存解析任务状态、代数、幂等和失败语义 | 创建/更新 | 任务必须绑定 Job | [`domain-model.md`](../../../domain/domain-model.md) | 待迁移 |
| ParsedJobDescriptionSnapshot | 保存成功结构化 JD 快照 | 成功时创建；失败不创建 | 通过 Job 归属读取 | [`domain-model.md`](../../../domain/domain-model.md) | 待迁移 |
...
- 扩展 `async_task_runs` 的任务类型/资源类型以支持 `job_jd_parse` / `job`；保存可脱敏的 `failure_semantics`、`failure_reason` 和任务代数；
...
| 依赖 | 用途 | 真实证据 | 状态 |
| --- | --- | --- | --- |
| PostgreSQL | Job、文件对象、任务和快照持久化 | 项目基础服务基线和 S-03 集成测试 | Readiness Check 待验证 |
| Redis/Celery/Dispatcher/Worker | 任务投递、执行、租约和有限重试 | S-03 任务运行验证 | Readiness Check 待验证 |
| 受控本地 JD 存储 | 内部验证 API 的输入文件 | 开发者准备的真实 Markdown JD | Verify 待验证 |
| Markdown Parser | 固定标题确定性解析 | S-03 单元测试和真实场景 | Implement/Verify 待验证 |
...
| --- | --- | --- | --- |
| 无 | 无 | 不适用 | 设计已收口，等待 Readiness Check 证据 |
...
| 数据设计 | 快照表、任务类型、失败语义和事务边界已同步 | 设计通过，迁移待实现 |
| API 安全 | 内部路径受控，公开前端不调用 | 设计通过，接口测试待实现 |
| 异步基础设施 | Dispatcher、Worker、租约和重试可运行 | 待项目环境真实验证 |
| 真实 JD 输入 | 受控根目录有脱敏 Markdown 演示文件 | 待开发者准备并验证 |
| 外部模型 | 本 Slice 不依赖 LLM | 通过 |

Readiness Check 在真实 PostgreSQL、Redis、Worker、Dispatcher 和内部 API 运行证据完成前，不得把后端状态标记为 `backend_ready`，也不得把 Integration Scenario 标记为 `integration_delivered`。

### 7.2 验证证据

| 验证类型 | 覆盖内容 | 结果 | 证据 |
| --- | --- | --- | --- |
| Schema | 成功 `fields` 五项核心字段和扩展字段 | 待实现 | `fields.schema.json` 为参考模板 |
| Service/Parser | 固定标题、原文、额外字段和核心字段完整性 | 待实现 | S-03 单元测试 |
| API / 异步 | 提交、查询、状态、幂等、重试和失败分类 | 待实现 | S-03 API/集成测试 |
| 前后端联调 | 本 Slice 不接入正式前端页面；验证内部场景 | 待实现 | `IS-S03-01` |

### 7.3 关闭结论

- `slice-spec.md` 与最终实现一致：待 Implement/Verify；
- 本文档与代码、迁移和测试一致：待 Implement/Verify；
- 全局领域、数据和业务事实已同步：Slice Design 已同步，迁移待实现；
- Handoff Contract 可供下游使用：契约已定义，待实现证据；
- Integration Contract 与真实 API 一致：待实现；
- Integration Scenario 演示步骤通过：待开发者真实调用；
- 未决开发者裁决：无；
- 最终结论：`pending Readiness Check`。
