...
当前 S-02 的业务裁决已同步至业务基线。API、Job 迁移和 S-03 任务技术字段属于 Coding Agent 的实现决策；跨端上传结果、自动上传交互和 `.md` 格式限制已由 `IC-S02-JD-UPLOAD@0.3` 锁定。S-02 页面只展示“上传成功”或“上传失败”，不展示“解析中”、岗位摘要或解析结果；查询、删除和岗位摘要不属于本次联调范围。解析失败 Job 再次上传属于 S-03 后续业务契约。
...
| 解析失败 Job | 不在 S-02 定义重传行为，等待 S-03 契约 |
...
| 删除 Job 后重传 | 已裁决 |
| 解析中 Job 重传 | 已裁决 |
| 解析失败 Job 重传 | S-03 后续定义，不阻塞 S-02 |
...
- S-03 真实解析和解析失败后重传不作为 S-02 自行裁决内容。
