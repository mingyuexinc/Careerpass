...
| S-07 Agent 投递启动 | 求职者点击启动 Agent | 仅在当前目标、当前简历、画像匹配资格和可用结构化 JD 均满足时，绑定当前简历并从未启动进入运行中 | S-03、S-04、S-06 | `missing` |
| S-08 岗位匹配与投递 | Agent 运行入口成立后执行匹配 | 系统使用本地简化算法生成结构化、可解释的匹配结果，并为成功匹配岗位创建系统内投递记录 | S-07（Handoff Contract：Agent 已进入运行中） | `missing` |
