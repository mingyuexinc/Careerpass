...
def test_reset_service_continues_remaining_claims_when_finalize_fails() -> None:
    first = CleanupClaim(uuid4(), "a" * 32, "ready")
    second = CleanupClaim(uuid4(), "b" * 32, "ready")

    class FailingOnceObjectRepository:
        def __init__(self) -> None:
            self.finalized: list[object] = []

        async def finalize_deletion(self, object_id: object) -> bool:
            if object_id == first.object_id:
                raise RuntimeError("finalize failed")
            self.finalized.append(object_id)
            return True

    objects = FailingOnceObjectRepository()
    storage = FakeStorage()
    service = DebugResetService(
        repository=FakeRepository(ResetResources((first, second))),
        object_repository=objects,  # type: ignore[arg-type]
        storage=storage,
        enabled=True,
    )
    ...
    assert storage.deleted == [first.storage_key, second.storage_key]
    assert objects.finalized == [second.object_id]
