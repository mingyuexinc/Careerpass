...
`.harness/changes` 用于记录每一项可追溯的设计、实现、验证和发布变更；不存放业务源代码、Alembic revision 或测试代码的副本。

根目录只允许状态目录、`_templates/`、`tools/` 和本说明文件。任何变更包均不得直接置于根目录。
...
- `released`：已实际上线、尚需保留近期追溯信息的变更。
- `archived`：已完成的历史变更、废弃提案或仅文档决策。

完成实现和验证但尚未发布的变更仍属于 `in-progress/`，不得因测试通过而直接进入 `released/`。
...
4. 实现、测试和发布过程只在对应阶段目录补充说明与证据；业务源代码、测试代码和 Alembic revision 仍存放在正式工程目录。
5. 实际上线后移动到 `released/`，完成追溯周期或确认不再需要活动维护后移动到 `archived/`。
